...
SERVER_CONFIG=/opt/bitbucket/conf/server.xml
...
if [ "$1" = "start" ]; then
    ...
    chown bitbucket:bitbucket  $SERVER_CONFIG
    
    exec /usr/local/bin/gosu bitbucket /opt/bitbucket/bin/start-bitbucket.sh -fg
